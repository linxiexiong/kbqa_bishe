...
def entity_linking():
    ...
    predict = clf.predict_proba(valid_features)
    #print predict
    sq_data_valid_with_ns['predict'] = predict[:, 1]
    sq_data_valid_with_ns['word_score_sum'] = sq_data_valid_with_ns.groupby('qid').agg({'word_score': sum})
    sq_data_valid_with_ns['word_score_norm'] = sq_data_valid_with_ns.apply(
            lambda x: x['word_score'] / x['word_score_sum'] if x['word_score_sum'] != 0 else 0, axis=1)

    print sq_data_valid_with_ns[0: 100]
    sq_data_valid_with_ns['score'] = sq_data_valid_with_ns.apply(
        lambda x: float(x['predict']) + float(x['word_score']), axis=1)

    hit1 = evaluate(sq_data_valid_with_ns, 1)
    hit5 = evaluate(sq_data_valid_with_ns, 5)
    hit10 = evaluate(sq_data_valid_with_ns, 50)
